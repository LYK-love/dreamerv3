...
class Agent(nj.Module):
  ...
  def policy(self, obs, state, mode='train'):
    '''
    Use the current DreamerV3 as a policy to generate next action, while updating the latent state.
    @obs: current infos of the env
    @state: current state of the env
    
    return:
    @outs: contains the predicted generated action
    @state: it's latent state and action parts will change.
    '''
    self.config.jax.jit and print('Tracing policy function.')
    obs = self.preprocess(obs) # compute the continuous
    (prev_latent, prev_action), task_state, expl_state = state
    embed = self.wm.encoder(obs) # flatten obs into embed, which is a vector. Here `obs` is not only image
    
    # Use wm to get next latent state.
    ...
    # Use a2c to get next action
    # task_state: the action distribution generated by the actor
    # task_state: will never change, don't know what it is
    ...
    if mode == 'eval':
      ...
      outs['action'] = outs['action'].sample(seed=nj.rng())  # Sample an action from the distribution
    elif mode == 'train':
      outs = task_outs
      outs['log_entropy'] = outs['action'].entropy()
      outs['action'] = outs['action'].sample(seed=nj.rng())
  ...
  def preprocess(self, obs):
    ...
    for key, value in obs.items():
      if key.startswith('log_') or key in ('key',):
        continue
      if len(value.shape) > 3 and value.dtype == jnp.uint8: # for 'image'
        value = jaxutils.cast_to_compute(value) / 255.0 # Kind of normalization?
      else:
        value = value.astype(jnp.float32)
      obs[key] = value
    obs['cont'] = 1.0 - obs['is_terminal'].astype(jnp.float32) # broadcasting.
    return obs
...
class WorldModel(nj.Module):
  ...
  def __init__(self, obs_space, act_space, config):
    ...
    # RSSM + encoder + [decoder, reward, continual]
    self.encoder = nets.MultiEncoder(shapes, **config.encoder, name='enc')
    self.rssm = nets.RSSM(**config.rssm, name='rssm')
    
    # MLP's shape = `()`, why?
    # Other arguments are from configs.
    self.heads = {
        'decoder': nets.MultiDecoder(shapes, **config.decoder, name='dec'),
        # 'decoder': MAEDecoder(shapes, **config.decoder, name='mae_dec'),
  ...
  def train(self, data, state):
    '''
    Dynamic learning. Train the world model (= RSSM + encoder + [decoder, reward, continual]).
    '''
    # modules = world model = RSSM + encoder + [decoder, reward, continual]
    modules = [self.encoder, self.rssm, *self.heads.values()]
    # modules = [self.rssm, *self.heads.values()] # Skip the encoder
